...
void CamPointLight_float(float3 WorldPosition, float3 WorldNormal, float3 WorldView, out float3 Lighting)
{
	float3 Col = float3(0, 0, 0);
	float exp = 0.5;
	float newDistanceAttenuation;
	...
	for (int i = 0; i < pixelLightCount; ++i)
	{
		...
		newDistanceAttenuation = sqrt(light.distanceAttenuation);

		float3 attenuatedLightColor = light.color * newDistanceAttenuation;

		Col += attenuatedLightColor;
	}
...
void CamExpansionPoints_float(float3 WolrdPosition, out float Poss)
{
	float Dist = 0;
	int _PointsLength;
	...
	for (int i = 0; i < 1; ++i)
	{
		...
		Dist += distance(float3(1,0,0), WolrdPosition);
